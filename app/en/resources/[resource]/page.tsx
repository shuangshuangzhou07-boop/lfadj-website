...
export function generateMetadata({ params }: { params: { resource: string } }) { return getPlaceholderMetadata("en", "resources", params.resource); }
export default function Page({ params }: { params: { resource: string } }) {
  const copy = getPlaceholderCopy("en", "resources", params.resource);
  if (!copy) notFound();

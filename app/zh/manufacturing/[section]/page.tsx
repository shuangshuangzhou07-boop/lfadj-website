...
export function generateMetadata({ params }: { params: { section: string } }) { return getPlaceholderMetadata("zh", "manufacturing", params.section); }
export default function Page({ params }: { params: { section: string } }) {
  const copy = getPlaceholderCopy("zh", "manufacturing", params.section);
  if (!copy) notFound();
